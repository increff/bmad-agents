...
class Installer {
  ...
  async installExpansionPacks(installDir, selectedPacks, spinner, config = {}) {
    ...
    for (const packId of selectedPacks) {
      ...
      try {
        ...
        // Create .env file if expansion pack has environment variables
        await this.createEnvFile(installDir, packId, pack, config);
  ...
  async createEnvFile(installDir, packId, pack, config) {
    const yaml = require('js-yaml');
    const fs = require('node:fs').promises;
    
    try {
      // Read the expansion pack's config.yaml to get env_var mappings
      const configPath = path.join(pack.path, 'config.yaml');
      if (!(await fileManager.pathExists(configPath))) {
        return; // No config, skip .env creation
      }

      const configContent = await fs.readFile(configPath, 'utf8');
      const packConfig = yaml.load(configContent);

      if (!packConfig.installation_questions || packConfig.installation_questions.length === 0) {
        return; // No questions, no env vars to create
      }

      // Collect env var mappings from installation questions
      const envVars = {};
      const answers = config.expansionPackAnswers?.[packId] || {};

      for (const question of packConfig.installation_questions) {
        if (question.env_var && answers[question.name]) {
          envVars[question.env_var] = answers[question.name];
        }
      }
      ...
      // Create or update .env file in the install directory
      const envPath = path.join(installDir, '.env');
      let existingEnvContent = '';
      
      // Read existing .env if it exists
      if (await fileManager.pathExists(envPath)) {
